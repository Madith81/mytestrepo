...
class SearchService
{
    ...
    protected function buildEntitySearchQuery($terms, $entityType = 'page')
    {
        ...
        if (count($terms['exact']) > 0) {
            $entitySelect->where(
                function (\Illuminate\Database\Eloquent\Builder $query) use ($terms, $entity) {
                    foreach ($terms['exact'] as $inputTerm) {
                        $query->where(
                            function (\Illuminate\Database\Eloquent\Builder $query) use ($inputTerm, $entity) {
    ...
    protected function applyTagSearch(\Illuminate\Database\Eloquent\Builder $query, $tagTerm)
    {
        ...
        $query->whereHas(
            'tags',
            function (\Illuminate\Database\Eloquent\Builder $query) use ($tagSplit) {
                $tagName = $tagSplit[1];
