zz# How to respond to Issues and PRs

## How we treat each other  

When replying to issues and PRs, make sure you always follow our [Code of Conduct](CODE_OF_CONDUCT.md) and our [vision for DocsPen](docs/VISION.md). Make sure to read these thoroughly and understand them before you interact with any other users! In general, be nice to each other, and treat **everyone** with the same respect and dignity.
...
Issues and PRs may get marked with labels to help the DocsPen team communicate with each other and with the community as a whole. Usually most issues and PRs will get two different labels, one for the tool it affects (e.g. `docspen`, `docspen_core`, `supply`, ...) and one that represents some general information about the state or nature of the issue/PR.

If you identify an issue that seems interesting but not time-critical, and is simple for new contributors to dive into, we recommend adding the “you can do this” label. PRs labeled “you can do this” should have very clear descriptions of the problem and solution. Remember that someone who is new to DocsPen will need some coaching to be successful!

### Workflow Labels

| Label | Meaning|
| ----- | ------ |
| awaiting-reply | The DocsPen team is engaged in discussion, but is currently waiting for a response from the community |
...
Each tool has it’s own label, e.g. `docspen` and `docspen_core`.

## How to review Pull Requests

### Recommended setup for testing the code

- Clone the _DocsPen_ repository by running  `git clone git@github.com:DocsPen/Platform.git` in your terminal
- For each PR you want to review, make sure to add the user’s fork as a remote
  - `git remote add <GITHUB_USERNAME> git@github.com:<GITHUB_USERNAME>/Platform.git`
...
- After checking out a user’s code, you should always make sure that the tests are still working.
  ...
  - Run `npm install` to make sure all Node dependencies are installed
  - Run `php artisan key:generate` to generate Environment key
...
### Using your _DocsPen_ clone in a project

Visit http://localhost in You Browser

If you want log to be displayed in terminal run `php artisan serve` and visit http://localhost:8000

### Reviewing the Code

Before diving into the source code changes of a pull request, step back and think if this change is a good change for _DocsPen_, and that it follows the [DocsPen vision](docs/VISION.md). If you are not 100% certain that a pull request adds good value to _DocsPen_, ask the author to clarify on why this should be included in the main code base, referring to the [VISION document](docs/VISION.md).
